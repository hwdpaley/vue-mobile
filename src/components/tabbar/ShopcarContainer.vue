<template>
  <div class="shopcar-container">
    ...
    <div class="goods-list">
      ...
      <!-- 商品列表项区域 -->
      <div class="mui-card" v-for="(item, i) in shopCarsList" :key="item.id">
...
export default {
  ...
  computed: {
    shopCarsList() {
      return this.$store.getters.getShopCars;
    }
  },
  mounted() {
    this.getShopCarsList();
  },
  methods: {
    getShopCarsList() {
      // 1. 获取到 store 中所有的商品的Id，然后拼接出一个 用逗号分隔的 字符串

      this.$store.commit("getShopCarsList");
    },
    remove(id, index) {
      // 点击删除，把商品从 store 中根据 传递的 Id 删除，同时，把 当前组件中的 goodslist 中，对应要删除的那个商品，使用 index 来删除
      // this.goodslist.splice(index, 1);
      this.$store.commit("removeFormGoodList", index);
